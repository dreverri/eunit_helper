...
walk_ast([], State) ->
    ...
    NewForms = [File, Module, espec(State), export(State)|Rest],
    case State#state.init of
        true ->
            NewForms ++ [espec_init(State)];
        false ->
            NewForms
    end;
...
walk_ast([Attr={function, _, espec_test_, 0, _}|Rest], State) ->
...
espec_init(State) ->
    {function,0,espec_test_,0,
        [{clause,0,[],[],
                [{call,0,
                        {remote,0,{atom,0,espec},{atom,0,init}},
                        [{atom,0,State#state.module}]}]}]}.

export(State) ->
    Funs = [Fun || {Fun, _} <- State#state.funs] ++ State#state.fixtures ++ [{espec_test_,0}],
